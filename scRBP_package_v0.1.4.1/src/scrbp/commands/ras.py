...
def register_subcommand(subparsers):
    ...
    p.add_argument("--out_format", choices=["csv", "loom", "both"], default="loom",
                   help="Which SC outputs to write (default: loom; RSS always CSV).")
    ...
    p.add_argument("--no-csv", dest="no_csv", action="store_true",
                   help="Force disable writing CSV even if --out_format includes csv.")
    p.add_argument("--no-loom", dest="no_loom", action="store_true",
                   help="Force disable writing LOOM even if --out_format includes loom.")
...
def main(args):
    ...
    # Use a fresh Python interpreter for each worker to avoid inheriting
    # locks from active background threads (e.g. tqdm/TimeBar), which can
    # deadlock when AUCell uses multiprocessing under fork.
    try:
    ...
    if args.aucell_in:
        ...
        # NOTE: do NOT wrap aucell() in the thread-based TimeBar — the tqdm
        # thread can survive into forked/spawned workers and deadlock them.
        auc_t0 = time.time()
        auc_raw = aucell(expr_df_full, regulons_list, num_workers=num_workers)
        print(f"[AUCell] completed in {time.time() - auc_t0:.2f}s")
